...
export default function ChatPage() {
  ...
  return (
    <div className='flex h-full w-full flex-col'>
      ...
      <div className='flex md768:px-6 px-4 md:px-12 pb-8'>
        <div className='relative flex-1'>
          <input
            value={input}
            onChange={e => setInput(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleSend()}
            className='w-full rounded-[20px] px-5 py-4 pr-32 focus:outline-none focus:ring-2 focus:ring-primary placeholder:text-sm placeholder:text-[#0000004D]'
            placeholder='Write a message...'
          />

          <button
            type='button'
            className='absolute md768:right-[8.5rem] right-[7.5rem] top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700'
          >
          ...
          <Button
            onClick={handleSend}
            className='absolute right-4 top-1/2 flex -translate-y-1/2 items-center gap-2 rounded-[20px] md768:px-5 md768:py-2 px-3 py-1'
          >
            Send
